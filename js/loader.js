...
var Loader = (function () {
	...
	window["setStageDimensions"]=function(x, y, w, h){
		...
		__config.w=w;
		__config.h=h;
		if(window["AVMPlayerCoolmath"]){
			window["AVMPlayerCoolmath"].setStageDimensions(x, y, w, h);
		}
		if(handleResize){
			handleResize();
		}
	...
	function runGame(progressEvent = f => f, completeEvent = f => f) {
		...
		runLoadingProcess(jss, bins, loadReporter.report, __config.debug).then(data => {
			const runner = window["startCoolmathGame"];
			if (!runner) {
			...
			// now coolmath player is available at window["AVMPlayerCoolmath"]
		...
		Object.assign(window, {
			updateCoolmathProgressBar: avmReporter.report,
			coolmathGameParseComplete: complete,
		});
	}
